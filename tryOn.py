...
def detectUpperBody(image):
    cascadePath = os.path.join(os.path.abspath(os.path.dirname(__file__)),"data/haarcascade_upperbody.xml")
    result = image.copy()
...
def add_sprite(img):
    ...
    image_path = img
    # print(img.rsplit('/',1))
    print("put_sprite",int(img.rsplit('/',1)[0][-1]))
...
def cvloop(run_event):
    ...
    model = os.path.join(os.path.abspath(os.path.dirname(__file__)),"data/shape_predictor_68_face_landmarks.dat")
    ...
    while run_event.is_set(): 
        ...
        for face in faces: 
            ...
            if SPRITES[3]:
                ...
                apply_sprite(image,image_path,w,x,y3-39, incl, ontop = False)

    
            (x0,y0,w0,h0) = get_face_boundbox(shape, 6) #bound box of mouth

            if SPRITES[4]: # ears for earings
            ...
            if SPRITES[5]:
                findRects = []
                upperPath = os.path.join(os.path.abspath(os.path.dirname(__file__)),"data/haarcascade_upperbody.xml")
                imageGray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
